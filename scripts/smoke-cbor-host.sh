...
# Framed canonical CBOR request: [1, "smoke", "ping", []]. The process first
# sends its ready frame and then the response. EOF ends the server cleanly.
actual=$(
  printf '\000\000\000\016\204\001\145smoke\144ping\200' |
    env CARP_DIR="$core" "$server" |
    od -An -tx1 |
    tr -d ' \n'
)
expected='0000000a8401657265616479f5010000000a840165736d6f6b65f5f6'
