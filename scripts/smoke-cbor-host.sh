...
# Framed canonical CBOR request: [2, "smoke", "ping", []]. The process first
# sends its ready frame and then the response. EOF ends the server cleanly.
actual=$(
  printf '\000\000\000\016\204\002\145smoke\144ping\200' |
    env CARP_DIR="$core" "$server" |
    od -An -tx1 |
    tr -d ' \n'
)
expected='0000000a8402657265616479f5020000000a840265736d6f6b65f5f6'
